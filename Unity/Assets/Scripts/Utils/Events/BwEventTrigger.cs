...
{
    public class BwEventTrigger : MonoBehaviour
    {
        ...
        private PointerEnterHandler pointerEnterHandler => _pointerEnterHandler
            ? _pointerEnterHandler
            : _pointerEnterHandler = gameObject.AddComponent<PointerEnterHandler>();

        private PointerExitHandler _pointerExitHandler;

        private PointerExitHandler pointerExitHandler => _pointerExitHandler
            ? _pointerExitHandler
            : _pointerExitHandler = gameObject.AddComponent<PointerExitHandler>();

        private PointerDownHandler _pointerDownHandler;

        private PointerDownHandler pointerDownHandler => _pointerDownHandler
            ? _pointerDownHandler
            : _pointerDownHandler = gameObject.AddComponent<PointerDownHandler>();

        private PointerUpHandler _pointerUpHandler;

        private PointerUpHandler pointerUpHandler => _pointerUpHandler
            ? _pointerUpHandler
            : _pointerUpHandler = gameObject.AddComponent<PointerUpHandler>();

        private PointerClickHandler _pointerClickHandler;

        private PointerClickHandler pointerClickHandler => _pointerClickHandler
            ? _pointerClickHandler
            : _pointerClickHandler = gameObject.AddComponent<PointerClickHandler>();

        private InitializePotentialDragHandler _initializePotentialDragHandler;

        private InitializePotentialDragHandler initializePotentialDragHandler => _initializePotentialDragHandler
            ? _initializePotentialDragHandler
            : _initializePotentialDragHandler = gameObject.AddComponent<InitializePotentialDragHandler>();

        private BeginDragHandler _beginDragHandler;

        private BeginDragHandler beginDragHandler => _beginDragHandler
            ? _beginDragHandler
            : _beginDragHandler = gameObject.AddComponent<BeginDragHandler>();

        private DragHandler _dragHandler;

        private DragHandler dragHandler =>
            _dragHandler ? _dragHandler : _dragHandler = gameObject.AddComponent<DragHandler>();

        private EndDragHandler _endDragHandler;

        private EndDragHandler endDragHandler => _endDragHandler
            ? _endDragHandler
            : _endDragHandler = gameObject.AddComponent<EndDragHandler>();

        private DropHandler _dropHandler;

        private DropHandler dropHandler =>
            _dropHandler ? _dropHandler : _dropHandler = gameObject.AddComponent<DropHandler>();

        private ScrollHandler _scrollHandler;

        private ScrollHandler scrollHandler =>
            _scrollHandler ? _scrollHandler : _scrollHandler = gameObject.AddComponent<ScrollHandler>();

        private UpdateSelectedHandler _updateSelectedHandler;

        private UpdateSelectedHandler updateSelectedHandler => _updateSelectedHandler
            ? _updateSelectedHandler
            : _updateSelectedHandler = gameObject.AddComponent<UpdateSelectedHandler>();

        private SelectHandler _selectHandler;

        private SelectHandler selectHandler =>
            _selectHandler ? _selectHandler : _selectHandler = gameObject.AddComponent<SelectHandler>();

        private DeselectHandler _deselectHandler;

        private DeselectHandler deselectHandler => _deselectHandler
            ? _deselectHandler
            : _deselectHandler = gameObject.AddComponent<DeselectHandler>();

        private MoveHandler _moveHandler;

        private MoveHandler moveHandler =>
            _moveHandler ? _moveHandler : _moveHandler = gameObject.AddComponent<MoveHandler>();

        private SubmitHandler _submitHandler;

        private SubmitHandler submitHandler =>
            _submitHandler ? _submitHandler : _submitHandler = gameObject.AddComponent<SubmitHandler>();

        private CancelHandler _cancelHandler;

        private CancelHandler cancelHandler =>
            _cancelHandler ? _cancelHandler : _cancelHandler = gameObject.AddComponent<CancelHandler>();
    }
}