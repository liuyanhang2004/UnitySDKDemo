...
@interface UnityAppController()<JFGameSDKDelegate>
...
- (NSUInteger)application:(UIApplication*)application supportedInterfaceOrientationsForWindow:(UIWindow*)window
{
    ...
    [[JFGameSDK shareInstance] application:application supportedInterfaceOrientationsForWindow:window];
...
+ (const char *)ConvertDictToJsonStr:(NSDictionary *)dict{
...
- (void)SDKLogin {
    NSLog(@"**************************JFGameSDK开始登录**************************");
    [[JFGameSDK shareInstance] logonWithController: _rootController isAutoLogon:YES delegate:self];
}
//登录回调
- (void)onLoginSuccess:(NSDictionary *)success{
    NSLog(@"**************************JFGameSDK登录成功**************************%@",success);
    UnitySendMessage("SDKMgr", "_login", "{\"code\":1}");
}
- (void)onLoginFailure:(NSDictionary *)failure{
    NSLog(@"**************************JFGameSDK登录失败**************************%@",failure);
    UnitySendMessage("SDKMgr", "_login", "{\"code\":3}");
}

//支付
- (void)SDKPlay:(const char *) roleId roleName:(const char *) roleName roleLevel:(const char *) roleLevel
    roleCTimeroleCTime:(const char *) roleCTimeroleCTime serveId:(const char *) serveId serveName:(const char *) serveName
        orderNo:(const char *) orderNo money:(long) money productId:(const char *) productId productName:(const char *) productName productDesc:(const char *)productDesc count:(int)count productUnit:(const char *)productUnit extendInfo:(const char *) extendInfo{
    ...
    //角色信息
    roleInfo.serverId = [NSString stringWithUTF8String: serveId];
    roleInfo.serverName = [NSString stringWithUTF8String: serveName];
    ...
    roleInfo.vipLevel = @"0";
    roleInfo.gameUserLevel = [NSString stringWithUTF8String: roleLevel];
    roleInfo.partyName = @"";
    roleInfo.creatTime = [NSString stringWithUTF8String: roleCTimeroleCTime];
    roleInfo.gameAreaId = [NSString stringWithUTF8String: serveId];
    roleInfo.fightPower = @"";//@"10000";
    roleInfo.profession = @"";//@"法师";
    //订单信息
    orderInfo.goodsID = [NSString stringWithUTF8String: productId];
    orderInfo.productName = [NSString stringWithUTF8String: productName];
    orderInfo.cpOrderID = [NSString stringWithUTF8String: orderNo]; //必填 游戏订单号
    orderInfo.count = count;  //必填 数量
    orderInfo.amount = [NSString stringWithFormat:@"%.2f", (double)money/100]; //必填 总价
    orderInfo.productDesc = [NSString stringWithUTF8String: productDesc];
    orderInfo.callbackUrl = @"http://dl-as.hotgamehl.com/hotgame/kd/newsdk/router/h/85OAL7mxxgtBYM4o/m/pay/s/552/p/85/f/pay/";
    orderInfo.extrasParams = [NSString stringWithUTF8String: orderNo];
    orderInfo.price = (double)money/count/100; // 单价？
    orderInfo.quantifier = @"";
    orderInfo.remarkInfo = [NSString stringWithUTF8String: extendInfo]; //@"160-2112261608911744193";
...
- (void)onPaySuccessCallback:(NSDictionary *)success{
    ...
    UnitySendMessage("SDKMgr", "_pay", [UnityAppController ConvertDictToJsonStr:result]);
}
...
- (void)SDKLogout{
    NSLog(@"**************************JFGameSDK退出登录**************************");
    [[JFGameSDK shareInstance] logoutWithController:_rootController delegate:self];
}
//退出登录回调
- (void)onLogoutLoginSuccess:(NSDictionary *)success{
    NSLog(@"**************************JFGameSDK退出登录**************************%@",success);
    UnitySendMessage("SDKMgr", "_logout", "{\"code\":1}");
}
- (void)onLogoutLoginFailure:(NSDictionary *)failure{
    NSLog(@"**************************JFGameSDK退出登录失败**************************%@",failure);
    UnitySendMessage("SDKMgr", "_logout", "{\"code\":2}");
}
@end
